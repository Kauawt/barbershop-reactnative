...
import { Service } from "../types"; // Certifique-se de que o tipo Service está definido conforme necessário.
...
const ServiceCard = ({ service, showBookButton = true }: ServiceCardProps) => {
  ...
  const handleBooking = () => {
    // Aqui você pode adicionar a lógica de navegação, por exemplo, para a página de agendamento
  ...
  return (
    <View className="bg-white rounded-lg shadow-lg overflow-hidden">
      <View className="h-40 overflow-hidden bg-barber-dark">
        <Image 
          source={{ uri: service.image || 'https://dummyimage.com/600x400/000/fff' }} // Usando imagem padrão
          style={{ width: '100%', height: '100%' }} // Usa styles padrão, omite resizeMode aqui
          resizeMode="cover" // Adiciona resizeMode diretamente
        />
      </View>
      <View className="p-4">
