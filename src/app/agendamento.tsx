...
export default function Agendamento() {
  ...
  const handleConfirm = () => {
    const serviceName = getSelectedServiceDetails()?.name;
    const barberName = getSelectedBarberDetails()?.name;
    const servicePrice = getSelectedServiceDetails()?.price;
    
    const confirmationMessage = `Serviço: ${serviceName}\nBarbeiro: ${barberName}\nData: ${formatDate(date)}\nHorário: ${time}\nValor: R$ ${servicePrice?.toFixed(2)}`;
    ...
    if (Platform.OS === 'web') {
      ...
      Alert.alert(
        ...
        `${confirmationMessage}\n\nVocê será redirecionado em 3 segundos...`,
        [{ text: "OK", onPress: () => setRedirectCountdown(3) }]
      );
    }
  };
            ...
            <>
              ...
              <View className="flex flex-row flex-wrap justify-center gap-3 mb-6">
                {barbers.map(barber => (
                  ...
                  >
                    <Text className={`text-center font-medium ${
                      selectedBarber === barber.id ? 'text-white' : 'text-gray-700'
                    }`}>
                      {barber.name}
                    </Text>
                  </TouchableOpacity>
            ...
            <>
              ...
              <View className="flex flex-row flex-wrap justify-center gap-3 mb-6">
                {['14:00', '15:00', '15:30'].map(hour => (
                  <TouchableOpacity
                    key={hour}
                    className={`rounded-lg p-3 w-24 items-center ${
                      time === hour ? 
                      'bg-yellow-500 shadow-md' : 
                      'bg-gray-100 hover:bg-gray-200'
                    } transition-colors`}
                  ...
                  >
                    <Text className={`font-medium ${
                      time === hour ? 'text-white' : 'text-gray-700'
                    }`}>
                      {hour}
                    </Text>
                  </TouchableOpacity>
